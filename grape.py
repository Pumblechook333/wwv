...
class Grape:
    ...
    def load(self, filename: str, n: int = 1):
        ...
        self.callsign = header_data[5][0].split()[-1]
    ...
    def dopRtPlot(self, figname, ylim=None, fSize=22, **kwargs):
        """
        Plot the doppler shift and relative power over time of the signal
        ...
        :return: .png plot into local repository
        """
        fSize = kwargs.get('fSize', self.plot_settings['fontsize'])
        pad = self.plot_settings['pad']
        ...
        for mode, f in enumerate(files):
            ...
            # Create the legend
            legend_labels = ["1-hop O", "2-hop O", "3-hop O", "4-hop O", "1-hop X", "2-hop X", "3-hop X", "4-hop X"]
            plt.legend(legend_labels, fontsize=fSize, loc='best', ncol=2)
    ...
    def rtBouncePlot(self, figname, ylim=None, **kwargs):
        """
        ...
        # Initializes paths to the data directories
        maxheight_data_dirs = ['C:/Users/sabas/Documents/GitHub/PHARvis/EXPORT/export_data_max_heights_57_O/',
                               'C:/Users/sabas/Documents/GitHub/PHARvis/EXPORT/export_data_max_heights_57_X/']
        initialelev_data_dirs = ['C:/Users/sabas/Documents/GitHub/PHARvis/EXPORT/export_data_initial_elevs_57_O/',
                                 'C:/Users/sabas/Documents/GitHub/PHARvis/EXPORT/export_data_initial_elevs_57_X/']
        
        # Initializes the file lists for the data directories
        maxheight_files = [os.listdir(maxheight_data_dirs[0]),
                           os.listdir(maxheight_data_dirs[1])]
        initialelev_files = [os.listdir(initialelev_data_dirs[0]),
                             os.listdir(initialelev_data_dirs[1])]
        
        # Determines number of hops in the data
        nhops = len(maxheight_files[0])

        # Initializes figure and time axis
        fig = plt.figure(figsize=(19, 10), layout='constrained')  # inches x, y with 72 dots per inch
        ax1 = fig.add_subplot(111)
        self.timeAxis(ax1, **kwargs)
        self.axcount += 1
        ...
        # Plot some dummy data to create the legend
        legpoints = []
        for mode in range(2):
            for i in range(4):
                if mode == 0:
        ...
        # Plot the reflection point data
        for mode in range(0, 2):  # Per mode
            for i, f_i in enumerate(range(0, nhops)):  # Per hop
                mh_df = pd.read_csv(maxheight_data_dirs[mode] + maxheight_files[mode][f_i], header=None)
                ie_df = pd.read_csv(initialelev_data_dirs[mode] + initialelev_files[mode][f_i], header=None)
        ...
        ax1.set_ylabel(f'Reflection Altitude (km) (R12 = {R12_number})', fontsize=fSize)
        ax1.set_ylim(ylim)  # -1 to 1 Hz for Doppler shift
        ax1.tick_params(axis='y', labelsize=fSize - 2, direction='out', pad=pad)
...
class GrapeHandler:
    ...
    def dopPlotOver(self, figname='dopPlotOver', fSize=22, ylim=None, **kwargs):
        ...
        plt.title('[%s %s %s | Midpoint %s %s | WWV %s %s] \n'
                  % (grape0.callsign,
